...
class AudioHelperTest < Test::Unit::TestCase
  ...
  def test_player_creates_container_for_flash_object
    player = media_player({}, :id => 'my_player')
    assert player.starts_with?(%(<div id="my_player_container">#{FLASH_REQUIRED_MESSAGE}</div>))
  end
